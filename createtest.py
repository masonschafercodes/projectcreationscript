...
class Folder(object):
    ...
    def remove_dir(self):
        ...
        self.fname = input("What directory should I remove: ")


        try:
            if not path.exists(f"{self.path_schema + '/' + self.fname}"):
                print("Directory Does Not Exist")
                return
            else:
                os.rmdir(f"{self.path_schema + '/' + self.fname}")
        except OSError:
            print(f"Deletion of directory {self.path_schema + '/' + self.fname} failed")
        else:
            print(f"Deletion of directory {self.path_schema + '/' + self.fname} successful")
...
path_test = Folder('masonschafercodes', 'KmDyLWbh9c27uKF')

path_test.create_dir()